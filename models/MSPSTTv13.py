...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x, x_mark):
        if x_mark is None:
            x = self.value_embedding(x) + self.position_embedding(x).unsqueeze(-2).unsqueeze(-2)
        else:
            x = self.value_embedding(x) + self.position_embedding(x).unsqueeze(-2).unsqueeze(-2) + self.temporal_embedding(x_mark).unsqueeze(-2).unsqueeze(-2)
    ...
    def __init__(self, configs, window_size=4): 
        ...
        self.encoder = MSPSTTEncoder(
                    [
                        ...
                        MultiScalePeriodicSpatialTemporalBlock(
                            FullAttention(
                            ...
                            learned_pe=configs.learned_pe,
                            qkv_bias=True,
                            qk_norm=False,
                            attn_drop=configs.dropout,
                            proj_bias=True,
                            proj_drop=configs.dropout,
                            mlp_drop=configs.dropout,
