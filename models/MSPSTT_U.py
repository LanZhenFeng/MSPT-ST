...
class MSPSTTDecoderLayer(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None, tau=None, delta=None):
        ...
        # parallel
        if self.parallelize:
            return self.forward_parallel(x, cross, x_mask=None, cross_mask=None, tau=tau, delta=delta)

        # serial
        else:
            return self.forward_serial(x, cross, x_mask=None, cross_mask=None, tau=tau, delta=delta)
...
class Model(nn.Module):
    ...
    def forward_one_step(self, x_enc, x_mark_enc):
        ...
        # encoder
        enc_out, _, balance_loss_1, downsamples = self.encoder(enc_out)
        # decoder
        dec_out, _, balance_loss_2 = self.decoder(enc_out, downsamples)
        # head
