...
class Model(nn.Module):
    ...
    def __init__(self, configs, N_S=2, N_T=8,
        ...
        self.model_type = configs.attn_type
        ...
        self.model_type = 'gsta' if self.model_type is None else self.model_type.lower()
        if self.model_type == 'incepu':
            self.hid = MidIncepNet(self.seq_len*self.hid_S, self.hid_T, N_T)
        else:
            self.hid = MidMetaNet(self.seq_len*self.hid_S, self.hid_T, N_T,
                input_resolution=(H, W), model_type=self.model_type,
                mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path)
    
    def forward_core(self, x):
        x_raw = rearrange(x, 'b t h w c -> b t c h w')
