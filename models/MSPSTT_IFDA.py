...
from layers.MSPSTT_Modules import WindowAttentionLayer, TemporalAttentionLayer, FullAttention, precompute_freqs_cis, apply_rotary_emb
...
class MSPSTTEncoderLayer(nn.Module):
    ...
    def forward_parallel(self, x, freq_cis=None, attn_mask=None):
        ...
        if self.pre_norm:
            ...
            x_t = x
        x_t = self.attention_t(x_t, x_t, x_t, freq_cis, attn_mask)
    ...
    def forward_serial(self, x, freq_cis=None, attn_mask=None):
        ...
        x = self.attention_t(x, x, x, freq_cis, attn_mask)
...
class MSPSTTEncoder(nn.Module):
    ...
    def forward(self, x, freq_cis=None, attn_mask=None):
        for encoder_layer in self.encoder_layers:
            x = encoder_layer(x, freq_cis, attn_mask)
...
class Model(nn.Module):
    ...
    def __init__(self, configs, window_size=4): 
        ...
        self.register_buffer("freqs_cis_enc", precompute_freqs_cis(configs.d_model, self.seq_len), persistent=False)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask_true=None):
        ...
        enc_out = self.encoder(enc_out, freqs_cis_enc)
        # decoder
