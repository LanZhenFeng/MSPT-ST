...
from layers.MSPSTT_Modules import MultiScalePeriodicAttentionLayer, WindowAttentionLayer, TemporalAttentionLayer, FullAttention, precompute_freqs_cis, apply_rotary_emb
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask_true=None):
        ...
        dec_mask = x_mark_dec[:, :self.label_len]
        for t in range(self.pred_len):
            dec_out = self.dec_embedding(dec_inp, dec_mask)
            dec_out = self.decoder(dec_out, enc_out, freqs_cis_dec)
            dec_inp = torch.cat([dec_inp, dec_out[:, -1:]], dim=1)
            dec_mask = torch.cat([dec_mask, x_mark_dec[:, self.label_len + t:self.label_len + t + 1]], dim=1)
